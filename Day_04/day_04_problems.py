...
Your puzzle answer was 25183.The first half of this puzzle is complete! It
...
def format_data(in_file: TextIOWrapper) -> list[str]:
    ...
    Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
    """
    data = [x.strip() for x in in_file.readlines()]
    for i, d in enumerate(data):
        index = d.find(":") + 2
        data[i] = d[index:].strip().split("|")
        for j, _ in enumerate(data[i]):
            data[i][j] = _.strip().replace("  ", " ").split(" ")
...
def part_1(data):
    ans = 0
    for d in data:
        ans += 2 ** max(0, (len(set(d[0]) & set(d[1])) - 1))
    return ans
