...
You're launched high into the atmosphere! The apex
of your trajectory just barely reaches the surface of a large island floating
in the sky. You gently land in a fluffy pile of leaves. It's quite cold, but
...
Your puzzle answer was 2406.The first half of this puzzle is complete! It
...
def format_data(in_file):
    ...
    for x in in_file.readlines():
        ...
        for i, draw in enumerate(rounds):
            rounds[i] = {"red": 0, "green": 0, "blue": 0}
            for draws in draw.split(", "):
                for draw in draws.split(","):
                    num, color = draw.split(" ")
                    rounds[i][color] = int(num)
            result[game_id] = rounds
    return result


def get_max_cubes(data):
    maxes = {}
    for key, val in data.items():
        maxes[key] = {"red": 0, "green": 0, "blue": 0}
        for d in val:
            for k, v in d.items():
                maxes[key][k] = v if maxes[key][k] < v else maxes[key][k]
...
def part_1(maxes):
    ...
    maxes = get_max_cubes(maxes)
    ans = set()
    for key, val in maxes.items():
        if (
            val["red"] <= rule["red"]
            and val["green"] <= rule["green"]
            and val["blue"] <= rule["blue"]
        ):
            ans.add(key)
    return sum(ans)
